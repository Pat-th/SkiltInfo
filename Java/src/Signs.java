...
class Signs {
    ...
    public List<JSONObject> getBoundingBox(double lat, double lon, int enum_id) throws Exception {
        ...
        HttpRequest request = HttpRequest.newBuilder()
                .GET()
                .uri(URI.create("https://nvdbapiles-v3.atlas.vegvesen.no/vegobjekter/96/?inkluder=alle&kartutsnitt="+latMin+","+lonMin+","+latMax+","+lonMax+"&srid=6173"))
                .setHeader("User-Agent", "Skiltinfo")
                .build();
        System.out.println("https://nvdbapiles-v3.atlas.vegvesen.no/vegobjekter/96/?inkluder=alle&kartutsnitt="+latMin+","+lonMin+","+latMax+","+lonMax+"&srid=6173");
